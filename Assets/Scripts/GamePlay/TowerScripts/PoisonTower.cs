...
public class PoisonTower : MonoBehaviour
{
    ...
    void Shoot()
    {
        ...
        PoisonBullet bullet = bulletGO.GetComponent<PoisonBullet>();
        if (bullet != null)
        {
            bullet.seekTarget(target);
            bullet.GetDamageValue(damage, poisonValue);
        }
    }

    void GetStats()
    {
        damage = instance.poisonTowerDamage;
        fireRate = instance.poisonTowerRate;
        range = instance.poisonTowerRange;
        poisonValue = instance.poisonValue;
    }
